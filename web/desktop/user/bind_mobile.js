...
Page({
	...
	submit: function(){
		var that =this;
		var api = '/_api/xpmsns/user/user/bindMobile';
		if ( that.validform(['company','contact_name', 'mobile', 'password', 'repassword','_vcode', 'smscode'], {_vcode:true, smscode:true}) == true ) {
			var data = that.formdata();
				...
				$.post(api, data, function( resp ) {
					...
					that.redirect(backurl, "手机号码绑定成功！", 5,  "后转向"+name+" <a href='"+backurl+"'>立即转向</a>");
	...
	formdata: function( fields ){
		fields = fields || ['company', 'contact_name', 'group_slug', 'mobile', '_vcode', 'repassword', 'password', 'smscode'];
		var data = {};
		for( var i in fields ) {
			var field = fields[i];
	...
	_check_company: function(){

		var data = this.formdata(['company']);
	...
	_check_contact_name: function(){

		var data = this.formdata(['contact_name']);
	...
	changeVcode: function(){
		var api = '/_api/xpmsns/user/user/vcode?width=200&height=32&size=20&t=' + Date.parse(new Date()); 
